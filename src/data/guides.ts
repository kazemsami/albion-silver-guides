...
const rawGuides: Omit<Guide, "reliability" | "defaultMarketCity" | "riskProfile">[] = [
  {
    ...
    tips: [
      ...
      "If you get knocked down, you keep your ore but still pay gear repair costs and lose your Pork Pie buff. Yellow zones do not loot your bag on knockdown.",
  ...
  },
  {
    ...
    tips: [
      ...
      "Stay in yellow zones for knockdown safety, blue zones are not more profitable, just less punishing on death than red. Knockdowns still wear down gather gear; budget small repair costs per hour.",
    ],
    profitBuild: {
      title: "Required Gear",
      description:
  ...
  },
  {
    ...
    requirements: [
      ...
      "Antiquarian's Den in any royal city (bank, repair, corrupted entrance) or corrupted portals in the open world",
      "Hunter (yellow/blue, knockdown, 700 base IP) to learn; Stalker (red/black, full loot, 900 base IP) for profit; Slayer (red/black, 1100 base IP, 100k+ infamy)",
    ...
    steps: [
      "Enter from the Antiquarian's Den in any royal city (map portal). Use the Den bank and repair station, then queue at the Corrupted Dungeon entrance when it opens (entrances cycle in waves for larger matchmaking pools). Open-world corrupted portals still work the same way.",
      "Find a Corrupted Dungeon entrance in the open world (sinister solo portal) or use a corrupted map from the market if you are not using the Den.",
    ...
    tips: [
      "Since the Abyssal Depths update, Corrupted Dungeons are easiest to reach from the Antiquarian's Den in each royal city. Den entrances open in waves, so you may wait briefly before entering.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Prioritize Rare, Epic, and Legendary nested Mists when scouting. T7 fishing zones show up most often in Epic and Legendary nests; Rare Mists can still roll T7 water but are less dependable.",
    ...
    tips: [
      ...
      "T7 fishing zones are most common in Epic and Legendary nested Mists. Rare nests can still have T7 water, but plan to scout for better nests rather than staying in Rare-only branches.",
  ...
  },
  {
    ...
    description:
      "Fish normal schools on T8 Avalonian Road maps for top-tier silver. Roads are full-loot black zones: the calculator models portal search time, banking frequency, gear deaths, and fish lost when you die with a full bag. Puremist Snapper is a separate RNG line. Reviewed against 2 logged Safe escape runs (~30 min each): ~924k/hr net extrapolated without Premium (Standard tax, fish + journal progress, after portal fees) and ~1.1M/hr with Premium on a cleaner road.",
    category: "fishing",
    difficulty: "advanced",
    zoneType: "dangerous",
    silverByLevel: [
      ...
      { label: "Greedy max profit (bank 30-40 min)", min: 900_000, max: 2_000_000 },
    ...
    requirements: [
      ...
      "Enter through the Brecilien Avalonian Road portal (recommended). Royal city portals work, but Brecilien is best for re-rolling bad roads.",
      "T3 bait every session (faster bites = more casts per hour)",
      "Pork Pie ×2 per hour on every fishing tab (+15% yield, +30% carry weight)",
      "Grandmaster fishing journal, ~114k net sell value per full journal (~150k full − 36k empty)",
    ],
    steps: [
      "Enter an Avalonian Road from Brecilien (recommended) or a yellow/blue zone royal portal. Travel inward until you find a T8 map with large water bodies.",
      ...
      "Safe setup: wear the escape build. Grandmaster fisherman cap, 2 Pork Pies per hour, rod and journal in bag. No fishing garb, boots, or invis potion. Sell raw fish from your bag; the Safe escape calculator uses the logged species mix, not chopped bycatch.",
    ...
    tips: [
      ...
      "Bank every 30-40 min on Safe escape, Normal, and Greedy. Roads are full-loot: one death with a full bag can wipe hours of profit.",
      "Use Safe escape for cheap mobility, Normal for T7 fisherman garb/boots, Greedy for full T8 set plus max fishing and spec.",
      "Portal time finding a good T8 road reduces effective fish/hr. Start from Brecilien when you can: logged runs used Brecilien returns to re-roll bad roads (~12k silver per return). Budget for 1-2 scouting returns per session.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Daily bulk (Mon-Wed): craft Major Healing Potion .1 with focus first. 72× Elusive Foxglove, 18× Goose Eggs, 18× Potato Schnapps, 45× Basic Arcane Extract per 5 pots (focus and 2,500 silver lab fee per batch; see calculator for your recipe's focus cost).",
      "Compare Major Energy Potion .1 and Poison Potion in the calculator before spending leftover focus. Energy uses 72× foxglove, 18× milk, 18× schnapps, 45× basic extract; Poison uses 24× foxglove, 12× teasel, 12× comfrey, 6× milk.",
      ...
      "Thu-Sat before CTAs: craft Major Gigantify .1, Major Resistance .1, and Major Sticky. Each makes 5 pots per batch with 72× mullein plus the foxglove, burdock, egg, milk, and corn hooch shown in the calculator.",
    ...
    tips: [
      "Focus per batch depends on your alchemy level, potions specialization, and focus efficiency. Numbers below were read from the craft UI at tested spec; yours will differ as spec rises.",
      "Tested focus per batch of 5 pots (at logged spec): T6 Major Healing .1 = 3,461, .2 = 4,895 (+ 2,500 silver lab fee), Major Energy .1 = 4,188, .2 = 5,923, Poison 1,635; T7 Major Gigantify .1 = 5,278, .2 = 7,009, Major Resistance .1 = 6,368, .2 = 8,103, Major Sticky 5,503.",
