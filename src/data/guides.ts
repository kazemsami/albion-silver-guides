...
const rawGuides: Omit<Guide, "reliability" | "defaultMarketCity" | "riskProfile">[] = [
  ...
  },
  {
    ...
    description:
      "Farm Lazygrass Plain near Bridgewatch on T5 harvester gear for mixed T3-T5 fiber and Expert cropper journal fills. Logged without Premium: ~140k gross per 30 min (~280k/hr before tax). With Premium on the same route, expect roughly ~450k/hr before tax.",
    category: "gathering",
    difficulty: "beginner",
    zoneType: "safe",
    ...
    requirements: [
      ...
      "Optional: Pork Pie (+15% yield, +30% carry weight) for higher stacks",
    ],
    steps: [
      "Base out of Bridgewatch. Farm Lazygrass Plain (yellow zone): circle the map for fiber nodes, steppe zones respawn quickly on a tight loop.",
    ...
    tips: [
      "Logged run (no Premium, Lazygrass Plain, ~30 min): ~225 T3 flax, ~250 T4 hemp, ~141 T5 skyflower, plus rare hemp and uncommon/rare skyflower when enchanted nodes spawned, one Expert journal filled. ~140k gross before tax (~280k/hr); ~125k net after Standard listing tax (~250k/hr).",
      ...
      "With Premium on the same Lazygrass Plain loop, expect roughly ~450k/hr before tax (+50% gather yield and lower listing tax). Toggle Premium in the header to compare.",
      ...
      "Pork Pie boosts yield and carry weight but was not used on the logged run. Add it when you want heavier bags, not as a baseline requirement.",
  ...
  },
  {
    ...
    requirements: [
      "Party of 4-5 for veteran targets on Avalonian Roads (4 players is common)",
      "One tracker with a T7 Tracking Toolkit (covers Golem, Dawnbird, Panther, and Werewolf via tier +1)",
      "Tracking Mastery plus target specializations for the veteran hunts you rotate",
      "Tank, healer, and 2 DPS in full-loot PvP gear for Roads black zones",
      "Beef Sandwich, Major Healing Potions, Major Gigantify, invisibility potions for Roads escapes",
    ],
    steps: [
      "Form a fixed 4-man group: 1 tracker (starts hunts), 1 tank (Incubus Mace + Guardian), 1 healer (Fallen Staff), 2 DPS (Wildfire Staff or Crystal Reaper). Share voice comms; track study and engage timing matter.",
  ...
  },
  {
    ...
    description:
      "Craft T6 and T7 bulk potions at an alchemy lab and compare recipes in the calculator. Profit is measured per 10,000 focus (with focus) or per batch (without focus). Listing tax and sell-through risk are included; station fees are not. Margins swing with buy prices and how fast stacks sell.",
    category: "crafting",
    difficulty: "intermediate",
    zoneType: "safe",
    ...
    steps: [
      ...
      "Daily bulk (Mon-Wed): craft Major Healing Potion with focus first. 72× Elusive Foxglove, 18× Goose Eggs, 18× Potato Schnapps per 5 pots (2,746 focus + 2,500 silver lab fee per batch).",
      "Compare Major Energy Potion (3,323 focus) and Poison Potion (1,635 focus) in the calculator before spending leftover focus. Energy uses 72× foxglove, 18× milk, 18× schnapps; Poison uses 24× foxglove, 12× teasel, 12× comfrey, 6× milk.",
      ...
      "Thu-Sat before CTAs: craft Major Gigantify (4,413 focus), Major Resistance (5,503 focus), and Major Sticky (5,503 focus). Each makes 5 pots per batch with 72× mullein plus the foxglove, burdock, egg, milk, and corn hooch shown in the calculator.",
      "List pots in Bridgewatch 1 silver under the lowest sell order in stacks of 50. Use Instant sell in the calculator for a quick floor, or Typical / Slow if you expect stacks to sit.",
    ...
    tips: [
      "Tested focus per batch of 5 pots: T6 Major Healing 2,746 (+ 2,500 silver lab fee), Major Energy 3,323, Poison 1,635; T7 Major Gigantify 4,413, Major Resistance 5,503, Major Sticky 5,503.",
      ...
      "The calculator defaults to With focus and Instant sell. Material return is 45% on focus crafts and 15% without focus. Toggle Without focus to compare filler batches.",
